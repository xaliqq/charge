...
import WebSliderAddModal from '../modals/web-slider-add-modal';
import WebSliderEditModal from '../modals/web-slider-edit-modal';
...
function WebSlider() {
  ...
  const changeSliderItemStatus = async (id: number) => {
    setDisableSwitch(true);
    try {
      await new SliderService().changeItemStatus(id);

      const obj: any = {
        ...sliderData,
        sliders: {
          ...sliderData?.sliders,
          datas: sliderData?.sliders?.datas?.map(item => {
            if (item?.id === id) {
        ...
        <Flex align="center">
          <Breadcrumb>
            <BreadcrumbItem>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbLink isCurrentPage href="/web/slider">
                Slayder
              </BreadcrumbLink>
            </BreadcrumbItem>
          </Breadcrumb>
