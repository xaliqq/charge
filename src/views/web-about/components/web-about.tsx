...
import WebAboutAddModal from '../modals/web-about-add-modal';
import WebAboutEditModal from '../modals/web-about-edit-modal';
import WebAboutHeader from './web-about-header';

interface IWebAboutFilter {
...
function WebAbout() {
  ...
  const [aboutData, setAboutData] = useState<IGetAboutResponse>();
  const [refreshComponent, setRefreshComponent] = useState<boolean>(false);
  const [queryParams, setQueryParams] = useState<IHTTPSParams[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [selectedItem, setSelectedItem] = useState<IWebAboutItem>({
    id: 0,
    title: '',
    description: '',
    imageUrl: '',
  ...
  const fetchData = async () => {
    ...
    try {
      const res: IGetAboutResponse = await new AboutService().getItems([
        ...queryParams,
      ...
      if (error instanceof AxiosError) {
        ...
        if (error?.response?.data?.messages?.length) {
          ...
          toast({
            title: 'Xəta baş verdi',
            description:
              'Haqqımızda məlumatlarının gətirilməsi zamanı xəta baş verdi',
            status: 'error',
            position: 'top-right',
            duration: 3000,
            isClosable: true
  ...
  const onSubmit: SubmitHandler<IWebAboutFilter> = async (
    data: IWebAboutFilter
  ) => {
    setCurrentPage(1);
    const queryParamsData: IHTTPSParams[] =
      convertFormDataToQueryParams<IWebAboutFilter>(data);
    setQueryParams(queryParamsData);
    setRefreshComponent(!refreshComponent);
  };

  const { handleSubmit, setValue, control } = useForm<IWebAboutFilter>({
    mode: 'onChange',
    defaultValues: {
      status: null,
      language: null
  ...
  const changeAboutItemStatus = async (id: number) => {
    setDisableSwitch(true);
    try {
      await new AboutService().changeItemStatus(id);

      const obj: any = {
        ...aboutData,
        datas: {
          ...aboutData?.datas,
          datas: aboutData?.datas?.datas?.map(item => {
            if (item?.id === id) {
      ...
      if (error instanceof AxiosError) {
        if (error?.response?.data?.messages?.length) {
          ...
          toast({
            title: 'Xəta baş verdi',
            description:
              'Haqqımızda məlumatlarının dəyişdirilməsi zamanı xəta baş verdi',
            status: 'error',
            position: 'top-right',
            duration: 3000,
            isClosable: true
  ...
  const deleteItem = async () => {
    ...
    try {
      await new AboutService().deleteItem(selectedItem?.id);
      toast({
        ...
        <Flex align="center">
          <Breadcrumb>
            <BreadcrumbItem>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbLink isCurrentPage href="/web/about">
                Haqqımızda
              </BreadcrumbLink>
            </BreadcrumbItem>
          </Breadcrumb>
      ...
      <Box mt={5} shadow="lg" bg="white" borderRadius={6} w="100%" p={4}>
        ...
        {!loading ? (
          <Box>
              ...
              <Table size="sm">
                <Thead textAlign="left">
                  <Tr>
                    ...
                    <Th textTransform="initial">İl</Th>
                ...
                {aboutData?.datas?.datas &&
                aboutData?.datas?.datas?.length > 0 ? (
                  <Tbody textAlign="left">
                    {aboutData?.datas?.datas?.map((z: IWebAboutItem) => (
            ...
            {aboutData?.datas?.datas && aboutData?.datas?.datas?.length > 0 ? (
              <Flex mt={4} justify="flex-end">
                <Pagination
                  currentPage={page}
                  totalCount={aboutData?.datas?.totalDataCount}
                  pageSize={10}
